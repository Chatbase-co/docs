...
<Note>
  **Multiple Registrations:** Calling `registerTools` multiple times will
  override previous actions. Make sure to provide all desired actions in a
  single `registerTools` call.
</Note>

<Note>
  **Environment Limitations:** Client-side custom actions will not work in the
  Chatbase Playground or Compare features, as these environments don't support
