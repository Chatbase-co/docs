...
There is no need to satisfy the Here’s an example of the embed code you’ll find on the dashboard:
...
```javascript
{
  data: { ... },  // Can be JSON object or string
  status: "success"
}
```
...
#### Calling `registerTools` multiple times

Calling `registerTools` multiple times will override previous actions. Therefore, you need to provide all desired actions in a single `registerTools` call.
